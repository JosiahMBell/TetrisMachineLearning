...
def get_parameters(board):
    ...
    for i in range(0, BOARDHEIGHT):     # Check each row
        line = True                     # Initialize a line check boolean
        for j in range(0, BOARDWIDTH):  # Check each column in the row
            if int(board[j][i]) == 0:   # If there is a blank space
                line = False            # It cannot be a line
        if line == True:                # If there is no blank space, line stays true
...
if __name__ == '__main__':
    ...
    while True:  # game loop
        ...
        if games_completed >= MAX_GAMES:
            ...
            plt.plot(game_index_array, weight2Array, label="Number of Lines")
